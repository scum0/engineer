...
{
    if (vt03_buf->ch0 < 364 || vt03_buf->ch0 > 1684)
        return PYRO_ERROR;
    if (!verify_crc16_check_sum(reinterpret_cast<uint8_t const *>(vt03_buf),
                                sizeof(vt03_buf_t)))
        return PYRO_ERROR;
    return PYRO_OK;
}
...
void vt03_drv_t::unpack(const uint8_t *buf)
{
    ...
    if (PYRO_OK == error_check(vt03_buf))
    {
        ...
        shared_v_rc.buttons.pause.update(vt03_buf->pause);

        shared_v_rc.mouse_axes.x =
            static_cast<float>(vt03_buf->mouse_x) / 32768.0f;
        shared_v_rc.mouse_axes.y =
            static_cast<float>(vt03_buf->mouse_y) / 32768.0f;
        shared_v_rc.mouse_axes.z =
            static_cast<float>(vt03_buf->mouse_z) / 32768.0f;

        shared_v_rc.buttons.press_l.update(vt03_buf->press_l != 0);
        shared_v_rc.buttons.press_r.update(vt03_buf->press_r != 0);
        shared_v_rc.buttons.press_m.update(vt03_buf->press_m != 0);
